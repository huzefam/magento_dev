...
class Magestore_Giftwrap_Adminhtml_GiftwrapController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction ()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('giftwrap')->__('Manage Gift Boxes'), 
        Mage::helper('giftwrap')->__('Manage Gift Boxes'));
        $this->renderLayout();
    }
    ...
    public function editAction ()
    {
        ...
        $model = Mage::getModel('giftwrap/giftwrap')->getStoreGiftwrap($id, 
        $store_id);
        ...
        try {
            ...
            if (! $this->getRequest()->getParam('id')) {
                ...
                if ($store_id != 0) {
                    ...
                    foreach ($arrFielName as $fielname) {
                        if ($giftwrapModel->getData('default_' . $fielname) ==
                         '1') {
                            $test[] = $fielname;
                            $count ++;
                            $giftwrapModel->setData($fielname, 
                            $giftwrapDefault->getData($fielname));
                        }
                    }
                    // var_dump($test);die();
                    ...
                    $giftwrapModel->save();
                    //Zend_Debug::dump($giftwrapModel->getData());die();
                    $arrFielName = array(0 => 'title', 1 => 'price', 
                    2 => 'character', 3 => 'image', 4 => 'personal_message', 
                    5 => 'status', 6 => 'sort_order');
                    foreach (Mage::getModel('core/store')->getCollection() as $store) {
                        ...
                        foreach ($arrFielName as $fielname) {
                            if ($giftwrapStore->getData('default_' . $fielname) ==
                             '1') {
                                $count ++;
                                $giftwrapStore->setData($fielname, 
                                $giftwrapModel->getData($fielname));
                            }
                        }
            ...
            if ($this->getRequest()->getParam('back')) {
                $this->_redirect('*/*/edit', 
                array('id' => $id, 'store' => $store_id));
                return;
            }
            $this->_redirect('*/*');
        } catch (Mage_Core_Exception $e) {
        	Zend_Debug::dump($e);die('1');
            $this->_getSession()->addError(nl2br($e->getMessage()));
            $this->_getSession()->setData('giftwrap_form_data', 
            $this->getRequest()
                ->getParams());
        } catch (Exception $e) {
        	Zend_Debug::dump($e->getMessage());die('12');
