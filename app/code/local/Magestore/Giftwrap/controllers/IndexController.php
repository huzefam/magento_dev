		...
		if($wraptype){
			...
			*/
			...
			if($wraptype == 1){
				...
				$wrapall = $data['wrap'];
				$blockPaper = Mage::getBlockSingleton('giftwrap/giftbox_paper');
				$qtyItem = $blockPaper->getNumberOfItemsCanWraped($item);
				if(floatval($qtyItem) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item,$data['giftbox_id']))){
			...
			if($wraptype == 2){
				...
				if($number_items > $maxItems){
					Mage::getSingleton('checkout/session')->addError('Number items in giftbox is too large!');
					$this->_redirect('checkout/cart');
					return ;
				}				
				/*if($data['giftbox_id'])
